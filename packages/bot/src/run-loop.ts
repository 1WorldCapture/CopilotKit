      ...
      if (!parsed.ok) {
        ...
        try {
          result = stringifyHandlerResult(
            await tool.handler(parsed.value as never, makeToolCtx(call) as never),
          );
        } catch (err) {
...
export function ensureAssistantToolCallMessage(
  ...
  const lastIsAssistantWithCalls =
    last !== undefined && last.role === "assistant" && Array.isArray(last.toolCalls);
...
export function pushToolResult(agent: AbstractAgent, toolCallId: string, content: string): void {
