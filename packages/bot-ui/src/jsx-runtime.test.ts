...
describe("jsx factory", () => {
  it("builds an IRNode with type and props", () => {
    const node = jsx("section", { children: "hi" }) as IRNode;
    expect(node.type).toBe("section");
    expect(node.props.children).toBe("hi");
  });
  it("jsxs keeps array children", () => {
    const node = jsxs("actions", { children: ["a", "b"] }) as IRNode;
    expect(Array.isArray(node.props.children)).toBe(true);
  });
  it("Fragment is a stable sentinel", () => {
    const node = jsx(Fragment, { children: ["a", "b"] }) as IRNode;
