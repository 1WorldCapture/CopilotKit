...
function fakeThread() {
  return {
    runAgent: vi.fn(async (_input?: { prompt?: string; context?: unknown }) => undefined),
...
describe("example slash commands", () => {
  ...
  it("/agent runs the agent with the command text as the prompt", async () => {
    ...
    await byName("agent").handler(ctx({ command: "agent", text: "why is prod down", thread: thread as never }));
    expect(thread.runAgent).toHaveBeenCalledTimes(1);
    expect(thread.runAgent.mock.calls[0]![0]).toMatchObject({ prompt: "why is prod down" });
  });

  it("/agent with no text posts usage and does not run the agent", async () => {
    const thread = fakeThread();
    await byName("agent").handler(ctx({ command: "agent", text: "", thread: thread as never }));
  ...
  it("/triage runs the agent with a triage prompt", async () => {
    const thread = fakeThread();
    await byName("triage").handler(ctx({ command: "triage", text: "", thread: thread as never }));
    expect(thread.runAgent).toHaveBeenCalledTimes(1);
    expect(String(thread.runAgent.mock.calls[0]![0]?.prompt)).toMatch(/triage/i);
  });
});
