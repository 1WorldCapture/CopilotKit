...
import { renderToIR, type IRNode } from "@copilotkit/bot-ui";
...
describe("confirm_write tool", () => {
  it("posts a ConfirmWrite picker and returns the resolved {confirmed:true}", async () => {
    ...
    const result = await confirmWriteTool.handler(
      { action: "Create Linear issue", detail: "CPK-9: Checkout 500s under load" },
    ...
    const { blocks, accent } = renderSlackMessage(renderToIR(awaited[0] as IRNode));
