...
export function ConfirmWrite({ action, detail }: ConfirmWriteProps) {
  return (
    <Message accent="#E2B340">
      ...
      <Context>{":lock:  Nothing is written until you click **Create**."}</Context>
      <Actions>
        <Button
          ...
          onClick={async ({ thread, message }: InteractionContext) => {
            await thread.update(
              ...
              <Message accent="#27AE60">
                <Header>{`✅ ${action}`}</Header>
                <Context>{":white_check_mark:  Approved — writing now."}</Context>
              </Message>,
            );
          }}
        >
          ...
          onClick={async ({ thread, message }: InteractionContext) => {
            await thread.update(
              ...
              <Message accent="#EB5757">
                <Header>{`🚫 ${action}`}</Header>
                <Context>{":no_entry_sign:  Declined — nothing was written."}</Context>
              </Message>,
            );
          }}
        >
